...
$aIssues = $client->api('issue')->all(array(
    'offset' => 0
    , 'query_id' => 131
    , 'limit' => 100
    , 'project_id' => 'sgdoce-qs-doc'
    , 'tracker_id' => 53
    , 'status_id' => '*'
    , 'cf_68' => 'Reunião'
        ));

$sCaminhoDestinoArquivo = 'out_os/';

if (!file_exists($sCaminhoDestinoArquivo)){
    mkdir($sCaminhoDestinoArquivo);
}

foreach ($aIssues['issues'] as $key => $value) {

    $aIssuesPai = $client->api('issue')->show($value['parent']['id']);
    //$aIssuesPai_subject = preg_replace('/[^\da-z ]/i', '',preg_replace( '/[`^~\'"]/', null, iconv( 'UTF-8', 'ASCII//TRANSLIT', $aIssuesPai['issue']['subject'])));
    $aIssuesPai_subject = preg_replace('/[^\da-z]/i', '', $aIssuesPai['issue']['subject']);
       
            
    //echo "key = " . $key . " subject  = " . $value['subject'] . " author = " . $value['author']['name'] . "\n";
    $sNomeArquivo =  "sgdoce_" . $value['id'] . "_Reuniao_" . $value['start_date'] . "_@" . $aIssuesPai_subject . "-" . $value['parent']['id'] . ".pdf";
    $sCaminhoDownloadArquivoPDF = "https://projeto.sgdoc.icmbio.gov.br/issues/".$value['id'].".pdf";
      
    $aFilePDF =  $client->getFile("/issues/".$value['id'].".pdf");
    
    echo "Salvando pdf da reuniao ".$sCaminhoDownloadArquivoPDF. " como ".$sCaminhoDestinoArquivo.$sNomeArquivo." \n\n";  
    \file_put_contents($sCaminhoDestinoArquivo.$sNomeArquivo,$aFilePDF);
       
}

echo "Total tiket de reuniao : " . $aIssues['total_count'] . " \n";
...
foreach ($aIssues['issues'] as $key => $value) {
    ...
    //echo "key = " . $key . " subject  = " . $value['subject'] . " author = " . $value['author']['name'] . "\n";
    $sNomeArquivo =  "sgdoce_" . $value['id'] . "_RS_" . preg_replace('/[^\da-z]/i', '', str_replace('FN', 'FN_', $value['subject']) ) . ".pdf";
    $sCaminhoDownloadArquivoPDF = "https://projeto.sgdoc.icmbio.gov.br/issues/".$value['id'].".pdf";
      
    $aFilePDF =  $client->getFile("/issues/".$value['id'].".pdf");
    
    echo "Salvando RS ".$sCaminhoDownloadArquivoPDF. " como ".$sCaminhoDestinoArquivo.$sNomeArquivo." \n\n";  
    \file_put_contents($sCaminhoDestinoArquivo.$sNomeArquivo,$aFilePDF);
       
}


echo "Total Req Software FN  : " . $aIssues['total_count'] . " \n";
...
// --> with ApiKey
//$client = new Redmine\Client('https://projeto.sgdoc.icmbio.gov.br', 'o57zF1CgirxbUWR5oHlG');
...
$client->setCheckSslCertificate(false);
